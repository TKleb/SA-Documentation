...
This Introduction is used to explain the basic functionalities of the tool used in some of the labs. In this case \acrfull{gdb}.
...
\begin{itemize}
    \item Learn about \acrshort{gdb} and how to use the different commands available
\end{itemize}
...
\subsubsection*{Choice of Topic}
Students working with Windows should also be given the opportunity to use a debugger. This lab will guide them through the \acrshort{gui} of the program and explain the basic functionality of what is coming up in future labs.
\subsubsection*{Objectives}
\begin{itemize}
...
\subsubsection*{Content}
This introduction explains all the relevant functionalities of \acrshort{ida} and how to install it.
\subsubsection*{Choice of Topic}
Some of the Reverse Engineers most powerful tools are disassemblers. We purposely chose \Gls{ida} over \Gls{ghidra} for the beginning. This prevents the students from just generating pseudo code instead of actually reading and understanding the assembly instructions in later labs. Because of this, the solutions of future labs are presented using \Gls{ida}.
\subsubsection*{Objectives}
\begin{itemize}
    \item Install \Gls{ida} and get a brief overview
    \item Learn to orient yourself in \acrshort{ida} and get ready to use it on a \gls{binary}.
\end{itemize}
\subsubsection*{Grading}
The introduction labs are not graded since they are only used as a lookup. They have a flag to make sure the students have read the tools.
...
\subsubsection*{Content}
In this lab the student will learn how to use \gls{ida} to statically reverse a given \gls{binary}. The goal is to find the main function in a \gls{binary} with and one without symbols. \\
This will show the student that seemingly simple things such as finding the main function can be a real problem if you have insufficient knowledge of the inner workings of a \gls{binary} and the automatic detection of \acrshort{ida} fails.
...
\subsubsection*{Objectives}
\begin{itemize}
    \item Find the main function in a normal \gls{binary}
    \item Find the main function in a \gls{binary} with symbols stripped
\end{itemize}
\subsubsection*{Grading}
The student has to inspect the main functions and find a \gls{flag} in both of them. The final hand-in will be the combined flag.
...
\end{itemize}
\subsubsection*{Grading}
The student has to use the newly aquired skills to find the \gls{flag} in a new binary using dynamic debugging skills.
...
\subsubsection*{Content}
This lab dives deeper into the reverse engineering rabbit hole and introduces a new concept. The student first starts a \gls{docker} container off a provided resource.
...
\subsubsection*{Content}
The student is presented a docker container which runs a webserver and exposes a port on which the script for the student to crack is running. The student first inspects and analyzes the script itself and then follows a walkthrough on how to write a simple script which exploits a common vulnerability in \acrshort{aes} \acrshort{ecb} mode. 
\subsubsection*{Choice of Topic}
